...
class Action:
    ...
    def _bulkadd(self):
        ...
        if len(ioc_array)>50:
            responses.send_message_to_slack(self._channel, 'Due to limitations at the current time, we are only allowing 50 IOCs to be pushed at once. Sorry for the inconvience.')
            return
        logger.debug(f'Going to submit {str(len(ioc_array))} IOCS...')
        responses.send_ioc_count(self._channel,len(ioc_array))
        count=0

        for i in ioc_array:
            with open('to_submit.txt','a+') as f:
                f.write(i+"\n")
